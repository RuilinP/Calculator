...
function resizeCanvas() {
  const topToolbarHeight = document.querySelector(".top-toolbar").offsetHeight;
  const bottomToolbarHeight =
    document.querySelector(".bottom-toolbar").offsetHeight;
...
document.getElementById("colorPicker").addEventListener("input", (e) => {
  ...
  document.getElementById("currentColor").textContent = `Color: ${color}`;
});
  ...
  return shapes.find((shape) => {
    switch (shape.type) {
      ...
      case "square":
        ...
        const squareXEnd =
          shape.startX + Math.sign(shape.endX - shape.startX) * squareSide;
        const squareYEnd =
          shape.startY + Math.sign(shape.endY - shape.startY) * squareSide;
        return (
          x >= shape.startX &&
          x <= squareXEnd &&
          y >= shape.startY &&
...
function moveObjects(dx, dy) {
  selectedShapes.forEach((shape) => {
    if (shape.type === "polygon") {
      shape.vertices = shape.vertices.map(([vx, vy]) => [vx + dx, vy + dy]);
    } else {
      shape.startX += dx;
      shape.startY += dy;
      shape.endX += dx;
